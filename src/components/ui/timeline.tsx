    ...
    <div className="w-full font-sans" ref={containerRef}>
      ...
      <div ref={ref} className="relative pb-16">
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-28 self-start max-w-xs md:w-full">
              ...
              <h4 className="hidden md:block text-lg md:pl-16 md:text-2xl font-semibold text-white/70">
                {item.title}
              </h4>
            </div>

            <div className="relative pl-16 pr-2 md:pl-4 w-full">
              <h4 className="md:hidden block text-lg mb-3 text-left font-semibold text-white/70">
                {item.title}
              </h4>
              {item.content}
            </div>
        ...
        <div
          ...
          className="absolute left-3 top-0 overflow-hidden w-[2px] bg-[linear-gradient(to_bottom,var(--tw-gradient-stops))] from-transparent via-white/20 to-transparent"
        >
          <motion.div
            style={{ height: heightTransform, opacity: opacityTransform }}
            className="absolute inset-x-0 top-0 w-[2px] bg-gradient-to-t from-white/80 via-white/40 to-transparent rounded-full"
          />
        </div>
      </div>
    </div>
